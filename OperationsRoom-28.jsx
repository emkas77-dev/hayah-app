...
function usePersistedState(key, initialValue, onStatus) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await Promise.resolve((() => { try { const v = localStorage.getItem(key); return v ? {value: v} : null; } catch { return null; } })());
  ...
  useEffect(() => {
    ...
    onStatus?.("saving");
    Promise.resolve((() => { try { localStorage.setItem(key, JSON.stringify(value)); } catch {} })())
      .then(() => onStatus?.("saved"))
