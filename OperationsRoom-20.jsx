...
// Simple state — no external storage needed
function usePersistedState(key, initialValue, onStatus) {
  const [value, setValue] = React.useState(() => {
    try {
      const saved = localStorage.getItem('hayah_' + key);
      return saved ? JSON.parse(saved) : initialValue;
    } catch { return initialValue; }
  });

  const setValueAndSave = React.useCallback((newVal) => {
    setValue(prev => {
      const next = typeof newVal === 'function' ? newVal(prev) : newVal;
      try { localStorage.setItem('hayah_' + key, JSON.stringify(next)); } catch {}
      return next;
    });
  }, [key]);
...
function CitizenPortal({ onEnterSystem }) {
  ...
  const services = [
    { id: "reading", icon: Activity, title: "رفع قراءة", desc: "سجّل قراءة عدادك", color: "#3BA0FF",
      fields: [{key:"account",ph:"رقم الحساب"},{key:"reading",ph:"القراءة الحالية"},{key:"phone",ph:"رقم هاتفك"}]},
    { id: "connect", icon: Wrench, title: "طلب توصيل", desc: "مياه أو صرف صحي", color: "#5FD68F",
      fields: [{key:"name",ph:"اسمك الكامل"},{key:"address",ph:"عنوانك"},{key:"phone",ph:"رقم هاتفك"}]},
    { id: "pay", icon: Wallet, title: "سداد فاتورة", desc: "ادفع إلكترونياً", color: "#E0A23B",
      fields: [{key:"account",ph:"رقم الحساب"},{key:"amount",ph:"المبلغ (ج.م)"},{key:"phone",ph:"رقم هاتفك"}]},
    { id: "complaint", icon: Headphones, title: "شكوى", desc: "أبلغ عن مشكلة", color: "#B89FFF",
  ...
  const waMsg = activeService ? encodeURIComponent(
    "مرحباً، أود متابعة طلبي (" + (services.find(s=>s.id===activeService)?.title||"طلب") + ")
رقمي: " + (form.phone||"")
  ) : "";

  return (
    <div dir="rtl" style={{ background: "#0A1628", minHeight: "100vh", fontFamily: "Cairo, sans-serif", display: "flex", flexDirection: "column" }}>
      <style>{FONT_IMPORT + `html{font-size:120%}*{font-family:Cairo,sans-serif;box-sizing:border-box}@keyframes slideDown{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:translateY(0)}}.slide-down{animation:slideDown 0.25s ease forwards}`}</style>

      {/* Header */}
      <div style={{ background:"rgba(255,255,255,0.04)", borderBottom:"1px solid rgba(255,255,255,0.08)", padding:"10px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <img src={COMPANY_LOGO} alt="شعار" style={{ width:32, height:32, borderRadius:"50%", objectFit:"cover" }} />
          <div>
            <p style={{ color:"#fff", fontSize:11, fontWeight:700, margin:0, lineHeight:1.2 }}>شركة مياه الشرب</p>
            <p style={{ color:"#7C93A6", fontSize:9, margin:0 }}>والصرف الصحي — كفر الشيخ</p>
          </div>
        </div>
        <button onClick={onEnterSystem} style={{ background:"linear-gradient(135deg,#3BA0FF,#1E6FCC)", color:"#fff", border:"none", borderRadius:16, padding:"6px 12px", fontSize:10, fontWeight:700, cursor:"pointer" }}>
          🔐 دخول الموظفين
        </button>
      </div>

      {/* News Ticker */}
      <div style={{ background:"#0D2137", borderBottom:"1px solid rgba(59,160,255,0.15)" }}>
        <NewsTickerBar />
      </div>

      {/* Hero Compact */}
      <div style={{ padding:"12px 16px", textAlign:"center", background:"linear-gradient(180deg,#0D2137,#0A1628)" }}>
        <img src={HAYAH_LOGO} alt="حياة" style={{ width:70, height:70, objectFit:"contain", filter:"drop-shadow(0 4px 12px rgba(59,160,255,0.4))", marginBottom:4 }} />
        <p style={{ color:"#fff", fontWeight:800, fontSize:15, margin:"0 0 2px" }}>في خدمتكم</p>
        <p style={{ color:"#7C93A6", fontSize:10, margin:"0 0 8px" }}>منصة مياه الشرب والصرف الصحي — كفر الشيخ</p>
        <div style={{ display:"flex", justifyContent:"center", gap:6, flexWrap:"wrap" }}>
          <a href="tel:125" style={{ background:"rgba(95,214,143,0.15)", color:"#5FD68F", border:"1px solid rgba(95,214,143,0.3)", borderRadius:16, padding:"4px 10px", fontSize:10, fontWeight:700, textDecoration:"none" }}>📞 125</a>
          <a href="https://wa.me/201098138383" target="_blank" style={{ background:"rgba(37,211,102,0.15)", color:"#25D366", border:"1px solid rgba(37,211,102,0.3)", borderRadius:16, padding:"4px 10px", fontSize:10, fontWeight:700, textDecoration:"none" }}>💬 واتساب</a>
          <a href="tel:01098138383" style={{ background:"rgba(59,160,255,0.15)", color:"#3BA0FF", border:"1px solid rgba(59,160,255,0.3)", borderRadius:16, padding:"4px 10px", fontSize:10, fontWeight:700, textDecoration:"none" }}>📱 01098138383</a>
        </div>
      </div>

      {/* Tabs */}
      <div style={{ display:"flex", padding:"8px 16px", gap:4 }}>
        {[["services","الخدمات"],["about","عن الشركة"],["contact","تواصل"]].map(([k,l]) => (
          <button key={k} onClick={() => setActiveTab(k)} style={{ flex:1, padding:"7px 4px", borderRadius:10, border:"none", cursor:"pointer", fontSize:11, fontWeight:700, background:activeTab===k?"#3BA0FF":"rgba(255,255,255,0.05)", color:activeTab===k?"#fff":"#7C93A6" }}>
            {l}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding:"0 16px 24px", flex:1 }}>
        ...
        {activeTab === "services" && (
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            {services.map((s) => (
              <div key={s.id}>
                <button onClick={() => toggleService(s.id)} style={{ width:"100%", background:activeService===s.id?:"rgba(255,255,255,0.04)", border:, borderRadius:activeService===s.id?"12px 12px 0 0":12, padding:"12px 8px", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:5, transition:"all 0.2s" }}>
                  <div style={{ width:40, height:40, borderRadius:"50%", background:, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <s.icon size={20} color={s.color} />
                  </div>
                  <p style={{ color:"#fff", fontWeight:700, fontSize:12, margin:0 }}>{s.title}</p>
                  <p style={{ color:"#7C93A6", fontSize:10, margin:0 }}>{s.desc}</p>
                  <span style={{ color:s.color, fontSize:12 }}>{activeService===s.id?"▲":"▼"}</span>
                </button>

                {activeService===s.id && !submitted && (
                  <div className="slide-down" style={{ background:, border:, borderTop:"none", borderRadius:"0 0 12px 12px", padding:10, display:"flex", flexDirection:"column", gap:7 }}>
                    {s.fields.map((f) => (
                      <input key={f.key} placeholder={f.ph} value={form[f.key]||""} onChange={e=>setForm({...form,[f.key]:e.target.value})}
                        style={{ background:"rgba(255,255,255,0.07)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:8, padding:"7px 10px", color:"#fff", fontSize:11, width:"100%", outline:"none" }} />
                    ))}
                    {s.id==="connect" && (
                      <select value={form.type||"water"} onChange={e=>setForm({...form,type:e.target.value})} style={{ background:"rgba(255,255,255,0.07)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:8, padding:"7px 10px", color:"#fff", fontSize:11 }}>
                        <option value="water">مياه شرب</option>
                        <option value="sewage">صرف صحي</option>
                      </select>
                    )}
                    {s.id==="complaint" && (
                      <select value={form.ctype||"lowPressure"} onChange={e=>setForm({...form,ctype:e.target.value})} style={{ background:"rgba(255,255,255,0.07)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:8, padding:"7px 10px", color:"#fff", fontSize:11 }}>
                    ...
                    <button onClick={handleSubmit} style={{ background:s.color, color:"#fff", border:"none", borderRadius:8, padding:"9px", fontSize:12, fontWeight:700, cursor:"pointer" }}>
                      إرسال ✉️
                    </button>
                  </div>
                )}
                ...
                {activeService===s.id && submitted && (
                  <div className="slide-down" style={{ background:"rgba(95,214,143,0.1)", border:"1px solid rgba(95,214,143,0.3)", borderTop:"none", borderRadius:"0 0 12px 12px", padding:10, textAlign:"center" }}>
                    <p style={{ color:"#5FD68F", fontWeight:700, fontSize:12, margin:"0 0 8px" }}>✅ تم الإرسال!</p>
                    <a href={} target="_blank" style={{ display:"inline-flex", alignItems:"center", gap:5, background:"#25D366", color:"#fff", borderRadius:8, padding:"7px 12px", fontSize:11, fontWeight:700, textDecoration:"none" }}>
                      💬 تابع عبر WhatsApp
                    </a>
                  </div>
                )}
        ...
        {activeTab === "about" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, background:"rgba(255,255,255,0.04)", borderRadius:14, padding:12 }}>
              <img src={COMPANY_LOGO} alt="شعار" style={{ width:52, height:52, borderRadius:"50%", objectFit:"cover" }} />
              <div>
                <p style={{ color:"#3BA0FF", fontWeight:700, fontSize:12, margin:"0 0 2px" }}>شركة مياه الشرب والصرف الصحي</p>
                <p style={{ color:"#7C93A6", fontSize:10, margin:0 }}>محافظة كفر الشيخ</p>
              </div>
            </div>
            <p style={{ color:"#CBD5E0", fontSize:11, lineHeight:1.7, margin:0, background:"rgba(255,255,255,0.03)", borderRadius:12, padding:12 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              {[{v:"75+",l:"محطة ورافع",i:"💧"},{v:"10",l:"محطات صرف",i:"🔵"},{v:"4,450+",l:"موظف متخصص",i:"👷"},{v:"3M+",l:"مواطن مخدوم",i:"👥"}].map((s,i) => (
                <div key={i} style={{ background:"rgba(255,255,255,0.04)", borderRadius:12, padding:"10px", textAlign:"center" }}>
                  <p style={{ fontSize:18, margin:"0 0 2px" }}>{s.i}</p>
                  <p style={{ color:"#3BA0FF", fontWeight:800, fontSize:15, margin:"0 0 2px" }}>{s.v}</p>
                  <p style={{ color:"#7C93A6", fontSize:10, margin:0 }}>{s.l}</p>
                </div>
              ))}
            </div>
            <div style={{ background:"rgba(59,160,255,0.05)", border:"1px solid rgba(59,160,255,0.15)", borderRadius:12, padding:12 }}>
              <p style={{ color:"#3BA0FF", fontWeight:700, fontSize:12, margin:"0 0 6px" }}>💡 نصائح ترشيد المياه</p>
              {["أغلق الصنبور أثناء غسيل الأسنان","أصلح التسريب فوراً — القطرة تهدر 20 لتر يومياً","استخدم الغسالة عند الحمل الكامل"].map((t,i) => (
                <p key={i} style={{ color:"#7C93A6", fontSize:11, margin:"3px 0", display:"flex", gap:6 }}><span style={{ color:"#3BA0FF" }}>✦</span>{t}</p>
              ))}
            </div>
          </div>
        )}
        ...
        {activeTab === "contact" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {[
              ...
              {href:"tel:01098138383",icon:"📱",label:"موبايل الدعم الفني",value:"01098138383",color:"#3BA0FF"},
              ...
              <a key={i} href={ct.href} target={ct.href.startsWith("http")?"_blank":undefined}
                style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", background:"rgba(255,255,255,0.04)", border:, borderRadius:14, textDecoration:"none" }}>
                <span style={{ fontSize:22 }}>{ct.icon}</span>
                <div>
                  <p style={{ color:"#7C93A6", fontSize:10, margin:0 }}>{ct.label}</p>
                  <p style={{ color:ct.color, fontWeight:700, fontSize:13, margin:0 }}>{ct.value}</p>
                </div>
              </a>
            ))}
          </div>
        ...
        <p style={{ textAlign:"center", color:"#2D3748", fontSize:10, marginTop:14 }}>© 2026 شركة مياه الشرب والصرف الصحي — كفر الشيخ</p>
      </div>
    </div>
  );
}
