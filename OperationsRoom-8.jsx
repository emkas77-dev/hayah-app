...
function CitizenPortal({ onEnterSystem }) {
  ...
  return (
    <div dir="rtl" style={{ background: c.bg, minHeight: "100vh", fontFamily: "Cairo, sans-serif" }}>
      ...
      <div className="flex items-center justify-between px-4 py-3" style={{ background: c.card, borderBottom: `1px solid ${c.line}` }}>
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: c.blue }}>
            <Droplets size={20} color={c.bg} />
          </div>
          <div>
            <p className="font-extrabold text-sm leading-tight" style={{ color: c.text }}>حياة</p>
            <p className="text-[10px]" style={{ color: c.soft }}>مياه كفر الشيخ</p>
          </div>
        </div>
        <button onClick={onEnterSystem}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold"
          style={{ background: c.blue, color: c.bg }}>
          <Shield size={14} /> دخول الموظفين
        </button>
      </div>
      ...
      {/* Hero */}
      <div className="px-4 py-6 text-center" style={{ background: `linear-gradient(180deg, #0D2137 0%, ${c.bg} 100%)` }}>
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-3" style={{ background: c.blue }}>
          <Droplets size={32} color={c.bg} />
        </div>
        <h1 className="font-extrabold text-xl mb-1" style={{ color: c.text }}>في خدمتكم</h1>
        <p className="text-sm" style={{ color: c.soft }}>شركة مياه الشرب والصرف الصحي — محافظة كفر الشيخ</p>
        <div className="flex justify-center gap-4 mt-4">
          <a href="tel:125" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold" style={{ background: "#173C2A", color: c.green }}>
            <Phone size={13} /> خط ساخن 125
          </a>
          <a href="tel:01098138383" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold" style={{ background: "#16314A", color: c.blue }}>
            <Phone size={13} /> 01098138383
          </a>
        </div>
      </div>
      ...
      <div className="px-4 pb-8 flex flex-col gap-5">
        {/* Services */}
        <div>
          <p className="font-bold text-sm mb-3" style={{ color: c.text }}>خدماتك في خطوة واحدة</p>
          <div className="grid grid-cols-2 gap-3">
            {services.map((s) => (
              <CitizenServiceCard key={s.id} icon={s.icon} title={s.title} desc={s.desc} color={s.color}
                onClick={() => { setActiveService(s.id); setSubmitted(false); setForm({}); }} />
            ))}
          </div>
        </div>
        ...
        {activeService && !submitted && (
          <div className="rounded-2xl p-4 flex flex-col gap-3" style={{ background: c.card, border: `1px solid ${c.line}` }}>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setActiveService(null)}><X size={18} color={c.soft} /></button>
            </div>

            {activeService === "reading" && (<>
              <input placeholder="رقم حسابك (مثال: AC-100234)" value={form.account||""} onChange={e=>setForm({...form,account:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle} />
              <input placeholder="القراءة الحالية للعداد" value={form.reading||""} onChange={e=>setForm({...form,reading:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle} />
              <input placeholder="رقم هاتفك" value={form.phone||""} onChange={e=>setForm({...form,phone:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle} />
            </>)}

            {activeService === "connect" && (<>
              <input placeholder="اسمك الكامل" value={form.name||""} onChange={e=>setForm({...form,name:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle} />
              <input placeholder="عنوانك بالتفصيل" value={form.address||""} onChange={e=>setForm({...form,address:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle} />
              <select value={form.type||"water"} onChange={e=>setForm({...form,type:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle}>
            ...
            {activeService === "pay" && (<>
              <input placeholder="رقم حسابك" value={form.account||""} onChange={e=>setForm({...form,account:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle} />
              <input placeholder="المبلغ (ج.م)" value={form.amount||""} onChange={e=>setForm({...form,amount:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle} />
              <input placeholder="رقم هاتفك" value={form.phone||""} onChange={e=>setForm({...form,phone:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle} />
            </>)}

            {activeService === "complaint" && (<>
              <input placeholder="اسمك" value={form.name||""} onChange={e=>setForm({...form,name:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle} />
              <select value={form.type||"lowPressure"} onChange={e=>setForm({...form,type:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle}>
              ...
              <input placeholder="عنوانك أو المنطقة" value={form.address||""} onChange={e=>setForm({...form,address:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle} />
              <input placeholder="رقم هاتفك" value={form.phone||""} onChange={e=>setForm({...form,phone:e.target.value})} className="px-3 py-2 rounded-lg text-sm" style={inputStyle} />
            </>)}

            <button onClick={handleSubmit} className="py-2.5 rounded-xl text-sm font-bold" style={{ background: c.blue, color: c.bg }}>
              إرسال الطلب
            </button>
          </div>
        )}
        ...
        {submitted && (
          <div className="rounded-2xl p-4 text-center" style={{ background: "#173C2A", border: `1px solid ${c.green}` }}>
            <p className="text-2xl mb-2">✅</p>
            <p className="font-bold" style={{ color: c.green }}>تم إرسال طلبك بنجاح!</p>
            <p className="text-sm mt-1" style={{ color: c.soft }}>سيتواصل معك فريق الخدمة قريباً على رقمك</p>
          </div>
        )}

        {/* About Company */}
        <div className="rounded-2xl p-4" style={{ background: c.card, border: `1px solid ${c.line}` }}>
          <p className="font-bold text-sm mb-3" style={{ color: c.blue }}>💧 نبذة عن الشركة</p>
          <p className="text-xs leading-relaxed" style={{ color: c.soft }}>
            شركة مياه الشرب والصرف الصحي بمحافظة كفر الشيخ — إحدى شركات قطاع الإسكان والمرافق العامة التابعة لوزارة الإسكان والمرافق والمجتمعات العمرانية. تتولى الشركة تشغيل وصيانة منظومة مياه الشرب وشبكات الصرف الصحي في جميع مراكز وقرى المحافظة البالغ عددها 10 مراكز، وتخدم ما يزيد عن 3 مليون مواطن.
          </p>
          <div className="grid grid-cols-2 gap-3 mt-3">
            {[
              { label: "محطات مياه وروافع", value: "75+" },
              { label: "محطات صرف صحي", value: "10" },
              { label: "موظف", value: "4,450+" },
              { label: "مركز خدمة", value: "10" },
            ].map((s, i) => (
              <div key={i} className="text-center py-2 rounded-xl" style={{ background: c.cardAlt }}>
                <p className="font-mono font-extrabold text-lg" style={{ color: c.blue }}>{s.value}</p>
                <p className="text-[11px]" style={{ color: c.soft }}>{s.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Contact */}
        <div className="rounded-2xl p-4 flex flex-col gap-3" style={{ background: c.card, border: `1px solid ${c.line}` }}>
          <p className="font-bold text-sm" style={{ color: c.text }}>تواصل معنا</p>
          <a href="tel:125" className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ background: c.cardAlt }}>
            <Headphones size={20} color={c.blue} />
            <div><p className="text-[11px]" style={{ color: c.soft }}>الخط الساخن</p><p className="font-mono font-bold" style={{ color: c.text }}>125</p></div>
          </a>
          <a href="tel:01098138383" className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ background: c.cardAlt }}>
            <Phone size={20} color={c.blue} />
            <div><p className="text-[11px]" style={{ color: c.soft }}>موبايل الدعم</p><p className="font-mono font-bold" style={{ color: c.text }}>01098138383</p></div>
          </a>
          <a href="mailto:emkas77@gmail.com" className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ background: c.cardAlt }}>
            <Mail size={20} color={c.blue} />
            <div><p className="text-[11px]" style={{ color: c.soft }}>البريد الإلكتروني</p><p className="font-mono font-bold text-sm" style={{ color: c.text }}>emkas77@gmail.com</p></div>
          </a>
        </div>

        {/* Water Tips */}
        <div className="rounded-2xl p-4" style={{ background: c.card, border: `1px solid ${c.line}` }}>
          <p className="font-bold text-sm mb-3" style={{ color: c.text }}>💡 نصائح ترشيد المياه</p>
          {[
            ...
            <p key={i} className="text-xs py-1.5 flex items-start gap-2" style={{ color: c.soft, borderBottom: i < 3 ? `1px solid ${c.line}` : "none" }}>
              <span style={{ color: c.blue }}>✦</span> {tip}
            </p>
          ))}
        </div>
