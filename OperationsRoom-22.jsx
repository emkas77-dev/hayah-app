...
function usePersistedState(key, initialValue, onStatus) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await new Promise(r => { try { const v = localStorage.getItem(key); r(v ? {value: v} : null); } catch { r(null); } });
  ...
  useEffect(() => {
    ...
    onStatus?.("saving");
    new Promise(r => { try { localStorage.setItem(key, JSON.stringify(value)); r(); } catch { r(); } })
      .then(() => onStatus?.("saved"))
...
function useLiveValues(baseValues) {
  return { values: baseValues, tick: 0 };
}
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
        {[
          ...
          { label: "محطات متوقفة", value: offlineCount, sub: `${Math.round(offlineCount/stations.length*100)}%`, color: c.red, icon: AlertTriangle },
          ...
          <div key={i} className="rounded-xl p-3 flex items-center gap-3" style={{ background: c.card, border: `1px solid ${c.line}` }}>
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: `${k.color}22` }}>
              <k.icon size={20} color={k.color} />
            </div>
            <div className="flex flex-col min-w-0">
              <div className="flex items-baseline gap-1.5">
                <span className="font-mono font-extrabold text-2xl leading-none" style={{ color: k.color }}>{k.value}</span>
                {k.sub && <span className="text-[11px] font-bold" style={{ color: k.color }}>{k.sub}</span>}
              </div>
              <span className="text-[10px] leading-tight mt-0.5 truncate" style={{ color: c.soft }}>{k.label}</span>
            </div>
          </div>
        ))}
      </div>
...
function LandingPage({ onEmployeeLogin }) {
  const [screen, setScreen] = useState("home"); // home | complaint | connect | reading | inquiry | pin
  const [form, setForm] = useState({});
  const [done, setDone] = useState(false);
  const [pin, setPin] = useState("");
  const [pinError, setPinError] = useState("");

  const inp = { background: "#122A40", border: "1px solid #1E3A52", borderRadius: 10, padding: "10px 14px", color: "#EAF2F8", fontSize: 14, width: "100%", outline: "none", fontFamily: "Cairo" };
  const btn = (bg, color = "#fff") => ({ background: bg, color, border: "none", borderRadius: 12, padding: "12px", fontSize: 14, fontWeight: 700, cursor: "pointer", width: "100%", fontFamily: "Cairo" });

  const submit = () => { setDone(true); setTimeout(() => { setDone(false); setScreen("home"); setForm({}); }, 3000); };

  const tryPin = () => {
    const v = pin.trim().toLowerCase().replace(/[٠-٩]/g, d => String.fromCharCode(d.charCodeAt(0) - 1584));
    if (v === ACCESS_CODE) { onEmployeeLogin(); } else { setPinError("الرقم السري غير صحيح"); }
  };

  if (screen === "pin") return (
    <div dir="rtl" style={{ background: "#0B1C2C", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "Cairo" }}>
      <style>{FONT_IMPORT}</style>
      <div style={{ width: 56, height: 56, borderRadius: 16, background: "#3BA0FF", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16, fontSize: 28 }}>🔐</div>
      <p style={{ color: "#EAF2F8", fontWeight: 700, fontSize: 18, margin: "0 0 4px" }}>دخول الموظفين</p>
      <p style={{ color: "#7C93A6", fontSize: 12, margin: "0 0 24px" }}>أدخل الرقم السري للدخول</p>
      <div style={{ width: "100%", maxWidth: 320, display: "flex", flexDirection: "column", gap: 12 }}>
        <input type="password" placeholder="الرقم السري" value={pin} onChange={e => { setPin(e.target.value); setPinError(""); }}
          onKeyDown={e => e.key === "Enter" && tryPin()} style={{ ...inp, textAlign: "center", fontSize: 18 }} autoFocus />
        {pinError && <p style={{ color: "#FF6B6B", fontSize: 12, textAlign: "center", margin: 0 }}>{pinError}</p>}
        <button onClick={tryPin} style={btn("#3BA0FF")}>دخول</button>
        <button onClick={() => setScreen("home")} style={btn("#122A40", "#7C93A6")}>رجوع</button>
      </div>
    </div>
  );

  return (
    <div dir="rtl" style={{ background: "#0B1C2C", minHeight: "100vh", fontFamily: "Cairo", display: "flex", flexDirection: "column" }}>
      <style>{FONT_IMPORT}{`html{font-size:100%}*{font-family:Cairo,sans-serif;box-sizing:border-box}`}</style>

      {/* Header */}
      <div style={{ background: "#0D2137", borderBottom: "1px solid #1E3A52", padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "#3BA0FF", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>💧</div>
          <div>
            <p style={{ color: "#EAF2F8", fontWeight: 700, fontSize: 13, margin: 0 }}>حياة</p>
            <p style={{ color: "#7C93A6", fontSize: 10, margin: 0 }}>شركة مياه كفر الشيخ</p>
          </div>
        </div>
        <button onClick={() => { setScreen("pin"); setPin(""); setPinError(""); }}
          style={{ background: "rgba(59,160,255,0.15)", color: "#3BA0FF", border: "1px solid rgba(59,160,255,0.3)", borderRadius: 20, padding: "6px 14px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
          🔐 دخول الموظفين
        </button>
      </div>
      ...
      <div style={{ padding: "20px 16px", flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ textAlign: "center", padding: "16px 0" }}>
          <p style={{ color: "#EAF2F8", fontWeight: 800, fontSize: 22, margin: "0 0 4px" }}>في خدمتكم</p>
          <p style={{ color: "#7C93A6", fontSize: 12, margin: "0 0 16px" }}>منصة مياه الشرب والصرف الصحي — محافظة كفر الشيخ</p>
          <a href="tel:125" style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#3BA0FF", color: "#fff", textDecoration: "none", borderRadius: 20, padding: "10px 24px", fontWeight: 700, fontSize: 14 }}>
            📞 الخط الساخن 125
          </a>
        </div>
        ...
        {screen === "home" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              { id: "complaint", icon: "📣", title: "تقديم شكوى", color: "#B89FFF" },
              { id: "connect", icon: "🔧", title: "طلب توصيل خدمة", color: "#5FD68F" },
              { id: "reading", icon: "📊", title: "تسجيل قراءة", color: "#3BA0FF" },
              { id: "inquiry", icon: "💳", title: "استعلام فاتورة", color: "#E0A23B" },
            ].map(s => (
              <button key={s.id} onClick={() => { setScreen(s.id); setForm({}); setDone(false); }}
                style={{ background: "#122A40", border: `1px solid ${s.color}44`, borderRadius: 16, padding: "20px 12px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 32 }}>{s.icon}</span>
                <p style={{ color: "#EAF2F8", fontWeight: 700, fontSize: 13, margin: 0 }}>{s.title}</p>
              </button>
            ))}
          </div>
        )}
        ...
        {/* Complaint Form */}
        {screen === "complaint" && !done && (
          <div style={{ background: "#122A40", borderRadius: 16, padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
            <p style={{ color: "#EAF2F8", fontWeight: 700, fontSize: 16, margin: 0 }}>📣 تقديم شكوى</p>
            <input placeholder="اسمك" value={form.name||""} onChange={e=>setForm({...form,name:e.target.value})} style={inp} />
            <select value={form.type||"lowPressure"} onChange={e=>setForm({...form,type:e.target.value})} style={inp}>
            ...
            <input placeholder="عنوانك أو المنطقة" value={form.address||""} onChange={e=>setForm({...form,address:e.target.value})} style={inp} />
            <input placeholder="رقم هاتفك" value={form.phone||""} onChange={e=>setForm({...form,phone:e.target.value})} style={inp} />
            <button onClick={submit} style={btn("#B89FFF")}>إرسال الشكوى</button>
            <button onClick={()=>setScreen("home")} style={btn("#0B1C2C", "#7C93A6")}>رجوع</button>
          </div>
        )}
        ...
        {screen === "connect" && !done && (
          <div style={{ background: "#122A40", borderRadius: 16, padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
            <p style={{ color: "#EAF2F8", fontWeight: 700, fontSize: 16, margin: 0 }}>🔧 طلب توصيل خدمة</p>
            <input placeholder="اسمك الكامل" value={form.name||""} onChange={e=>setForm({...form,name:e.target.value})} style={inp} />
            <select value={form.type||"water"} onChange={e=>setForm({...form,type:e.target.value})} style={inp}>
              <option value="water">توصيل مياه شرب</option>
              <option value="sewage">توصيل صرف صحي</option>
            </select>
            <input placeholder="عنوانك بالتفصيل" value={form.address||""} onChange={e=>setForm({...form,address:e.target.value})} style={inp} />
            <input placeholder="رقم هاتفك" value={form.phone||""} onChange={e=>setForm({...form,phone:e.target.value})} style={inp} />
            <button onClick={submit} style={btn("#5FD68F", "#0B1C2C")}>إرسال الطلب</button>
            <button onClick={()=>setScreen("home")} style={btn("#0B1C2C", "#7C93A6")}>رجوع</button>
          </div>
        )}
        ...
        {screen === "reading" && !done && (
          <div style={{ background: "#122A40", borderRadius: 16, padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
            <p style={{ color: "#EAF2F8", fontWeight: 700, fontSize: 16, margin: 0 }}>📊 تسجيل قراءة العداد</p>
            <input placeholder="رقم حسابك" value={form.account||""} onChange={e=>setForm({...form,account:e.target.value})} style={inp} />
            <input placeholder="القراءة الحالية (م³)" value={form.reading||""} onChange={e=>setForm({...form,reading:e.target.value})} style={inp} />
            <input placeholder="رقم هاتفك" value={form.phone||""} onChange={e=>setForm({...form,phone:e.target.value})} style={inp} />
            <button onClick={submit} style={btn("#3BA0FF")}>تسجيل القراءة</button>
            <button onClick={()=>setScreen("home")} style={btn("#0B1C2C", "#7C93A6")}>رجوع</button>
          </div>
        )}
        ...
        {screen === "inquiry" && !done && (
          <div style={{ background: "#122A40", borderRadius: 16, padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
            <p style={{ color: "#EAF2F8", fontWeight: 700, fontSize: 16, margin: 0 }}>💳 استعلام عن فاتورة</p>
            <input placeholder="رقم حسابك" value={form.account||""} onChange={e=>setForm({...form,account:e.target.value})} style={inp} />
            <input placeholder="اسمك" value={form.name||""} onChange={e=>setForm({...form,name:e.target.value})} style={inp} />
            <input placeholder="رقم هاتفك" value={form.phone||""} onChange={e=>setForm({...form,phone:e.target.value})} style={inp} />
            <button onClick={submit} style={btn("#E0A23B", "#0B1C2C")}>استعلام</button>
            <button onClick={()=>setScreen("home")} style={btn("#0B1C2C", "#7C93A6")}>رجوع</button>
          </div>
        )}
        ...
        {/* Success */}
        {done && (
          <div style={{ background: "#173C2A", border: "1px solid #5FD68F", borderRadius: 16, padding: 24, textAlign: "center" }}>
            <p style={{ fontSize: 48, margin: "0 0 8px" }}>✅</p>
            <p style={{ color: "#5FD68F", fontWeight: 700, fontSize: 18, margin: "0 0 4px" }}>تم الإرسال بنجاح!</p>
            <p style={{ color: "#7C93A6", fontSize: 12, margin: "0 0 16px" }}>سيتواصل معك فريق الخدمة قريباً</p>
            <a href={`https://wa.me/201098138383?text=${encodeURIComponent("مرحباً، أود متابعة طلبي المُسجَّل عبر منصة حياة\nرقمي: " + (form.phone||""))}`}
              target="_blank" rel="noreferrer"
              style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#25D366", color: "#fff", textDecoration: "none", borderRadius: 12, padding: "12px 24px", fontWeight: 700, fontSize: 14 }}>
              💬 تابع عبر WhatsApp
            </a>
          </div>
        )}
        ...
        {screen === "home" && (
          <div style={{ display: "flex", gap: 10 }}>
            <a href="tel:01098138383" style={{ flex: 1, background: "#122A40", border: "1px solid #1E3A52", borderRadius: 12, padding: "12px", textAlign: "center", textDecoration: "none" }}>
              <p style={{ fontSize: 20, margin: "0 0 4px" }}>📱</p>
              <p style={{ color: "#3BA0FF", fontWeight: 700, fontSize: 12, margin: 0 }}>01098138383</p>
            </a>
            <a href="https://wa.me/201098138383" target="_blank" rel="noreferrer" style={{ flex: 1, background: "#122A40", border: "1px solid #1E3A52", borderRadius: 12, padding: "12px", textAlign: "center", textDecoration: "none" }}>
              <p style={{ fontSize: 20, margin: "0 0 4px" }}>💬</p>
              <p style={{ color: "#25D366", fontWeight: 700, fontSize: 12, margin: 0 }}>واتساب</p>
            </a>
            <a href="mailto:emkas77@gmail.com" style={{ flex: 1, background: "#122A40", border: "1px solid #1E3A52", borderRadius: 12, padding: "12px", textAlign: "center", textDecoration: "none" }}>
              <p style={{ fontSize: 20, margin: "0 0 4px" }}>📧</p>
              <p style={{ color: "#B89FFF", fontWeight: 700, fontSize: 12, margin: 0 }}>إيميل</p>
            </a>
          </div>
        )}

        <p style={{ textAlign: "center", color: "#1E3A52", fontSize: 11 }}>© 2026 شركة مياه الشرب والصرف الصحي — كفر الشيخ</p>
      </div>
    </div>
  );
}
...
export default function OperationsRoom() {
  ...
  return landed ? <OperationsRoomInner /> : <LandingPage onEmployeeLogin={() => setLanded(true)} />;
}
